...
esp_err_t fileRead(void)
{
    ...
    if (pFile != NULL)
    {
        ...
        if (strcmp(ans,pswd) == 0)
        {
            if (configFile == NULL && newFile == NULL)
            {
                for (; fl < 10; fl++)
                {
                    ...
                    vTaskDelay(250/portTICK_PERIOD_MS);gpio_set_level(BK, 1);
                    vTaskDelay(250/portTICK_PERIOD_MS);gpio_set_level(BK, 0);
                }                      
            ...
            else
            {
                ...
                if(configFile != NULL && flag1 == true)
                {
                    ...
                    { 
                        ...
                        // value[i] = (int32_t)strtol(lines[i], NULL, 10);   
                        value[i] = (int32_t)(strtof(lines[i], NULL)*100.0);  
                        // printf("O - %" PRId32 "\n",value[i]);
                    ...
                    { 
                        nvs_open("value_s"+i, NVS_READWRITE,&myHandle.value_r);
                        // value[i] = (int32_t)strtol(lines[i+1], NULL, 10);   
                        value[i] = (int32_t)(strtof(lines[i+1], NULL)*100.0); 
                        // printf("C - %" PRId32 "\n",value[i]); 
                        nvs_set_i32(myHandle.value_r, "abab"+i, (int32_t)value[i]);      
                        nvs_commit(myHandle.value_r);   
                        nvs_close(myHandle.value_r);
        ...
        for (uint8_t i = 0; i < sizeof(value1)/sizeof(value1[0]); i++)
        {
            nvs_open("value_s"+i+6, NVS_READONLY, &mdHde.value_mid1);
            nvs_get_i32(mdHde.value_mid1, "abab"+i+6, (int32_t*)&value1[i]);
            // printf("get3 - %" PRId32 "\n",value[i]);
            // printf("get3 - %.2f\n",(value[i]/100.0));
            nvs_close(mdHde.value_mid1);
        }
