...
{
	httpd_resp_send(req, (const char *) index_html_start, index_html_end - index_html_start);return ESP_OK;
}

esp_err_t jpg_get_handler(httpd_req_t *req)
{
	httpd_resp_send(req, (const char *) eita_jpg_start, eita_jpg_end - eita_jpg_start);return ESP_OK;
}
/*
...
esp_err_t data_get_open_handler(httpd_req_t *req)
{
	...
	for (j = 0; j < (sizeof(opntemp1)/sizeof(opntemp1[0]))-4; ++j)
	{
		nvs_open("value_s"+j, NVS_READWRITE, &c_opn.c_value_opn);
		value[j] = (int32_t)(strtof(opntemp1[j],NULL)*100.0);
	...
	for (j = 0; j < (sizeof(opntemp1)/sizeof(opntemp1[0]))-4; ++j)
	{
		nvs_open("value_s"+j+6, NVS_READWRITE, &c_opn.c_value_opn1);
		value1[j] = (int32_t)(strtof(opntemp1[j+3],NULL)*100.0);
		nvs_set_i32(c_opn.c_value_opn1, "abab"+j+6, (int32_t)value1[j]);
		nvs_commit(c_opn.c_value_opn1);
		nvs_close(c_opn.c_value_opn1);
	}
	nvs_open("time_s"+0, NVS_READWRITE,&c_opn.time_mid_opn);
	time1[0] = atoi(opntemp1[6]);           
...
esp_err_t data_get_close_handler(httpd_req_t *req)
{
	...
	for (k = 0; k < (sizeof(clstemp1)/sizeof(clstemp1[0]))-4; ++k)
	{
		nvs_open("value_s"+k+3, NVS_READWRITE, &c_cls.c_value_cls);
		value[k+3] = (int32_t)(strtof(clstemp1[k],NULL)*100.0);
	...
	for (k = 0; k < (sizeof(clstemp1)/sizeof(clstemp1[0]))-4; ++k)
	{
		nvs_open("value_s"+k+9, NVS_READWRITE, &c_cls.c_value_cls1);
		value1[k+3] = (int32_t)(strtof(clstemp1[k+3],NULL)*100.0);
		nvs_set_i32(c_cls.c_value_cls1, "abab"+k+9, (int32_t)value1[k+3]);
		nvs_commit(c_cls.c_value_cls1);
		nvs_close(c_cls.c_value_cls1);
	}
	nvs_open("time_s"+1, NVS_READWRITE,&c_cls.time_mid_cls);
	time1[1] = atoi(clstemp1[6]);           
...
esp_err_t data_show_handler(httpd_req_t *req)
{
	...
	for (int n = 0; n < (sizeof(value1)/sizeof(value1[0])); n++)
	{
		nvs_open("value_s"+n+6, NVS_READONLY, &showhdl.value_mid1);
		nvs_get_i32(showhdl.value_mid1, "abab"+n+6, (int32_t*)&value1[n]);
		nvs_close(showhdl.value_mid1);
	}
	...
	snprintf(dataShow, sizeof(dataShow), "<center><p><strong>Open --> H:%.2f S:%.2f V:%.2f T:%d ms<br>H1:%.2f S1:%.2f V1:%.2f<br>Close --> H:%.2f S:%.2f V:%.2f T:%d ms<br>H1:%.2f S1:%.2f V1:%.2f<br></strong></p></center>", value[0]/100.0, value[1]/100.0, value[2]/100.0, time1[0], value1[0]/100.0, value1[1]/100.0, value1[2]/100.0, value[3]/100.0, value[4]/100.0, value[5]/100.0, time1[1], value1[3]/100.0, value1[4]/100.0, value1[5]/100.0);
...
void app_main(void)
{
	...
	{
        // xTaskCreatePinnedToCore(midopn, "opn", 4096, NULL, 1, NULL, 0);
        // xTaskCreatePinnedToCore(clsmid, "cls", 4096, NULL, 1, NULL, 0);
        xTaskCreatePinnedToCore(fcob, "fcob", 10000, NULL, 1, NULL, 0); 
